...
Configuración en profundidad (appsettings) — claves Gemini y GitHub



Docker Compose — guía principal
...
Alcance, limitaciones y decisiones
...
Host: DI, pipeline HTTP, Serilog, OpenTelemetry, carga de configuración.
...
Controladores, modelos HTTP, middlewares compartidos.
...
Casos de uso (MediatR), validación, contratos de aplicación.
...
Servicios externos (Gemini, HTTP clients), autenticación JWT, etc.
...
EF Core, repositorios, migraciones, seeding en desarrollo/Docker.
...
Entidades, value objects, errores de dominio, eventos.
...
dotnet run, Visual Studio con perfil local
...
Contenedor definido en Compose (entorno Docker fijado en docker-compose.override.yml)
...
Clave simétrica HS256; debe tener longitud suficiente (p. ej. ≥ 32 caracteres). No commitear valores reales en repositorios públicos.
...
Aplica migraciones EF al iniciar (solo Development / Docker en el código actual).
...
Crea roles de Identity (p. ej. User, Admin).
...
Crea usuarios desde DevelopmentUsers si no existen.
...
Datos demo (p. ej. topics/jokes de prueba) si aplica.
...
Lista de { Name, Email, Password, Role } para seed.
...
ConnectionStrings:ConnectionEnjoy
...
Clave de la API Gemini (Google AI)
...
Id de modelo en la API Gemini (Google): Gemini o Gemma, según la doc. oficial





ExternalAuth:GitHub:ClientId / ClientSecret / RedirectUri
...
Debe ser un id de modelo válido de la API Gemini (Google AI), ya sea familia Gemini o Gemma listada para esa API: copia el identificador exacto de la documentación de modelos (p. ej. gemini-2.0-flash o gemma-3-12b-it). Enjoy no traduce ni mapea ese valor: es el string que la API espera en las peticiones.
...
Entrar en la configuración de desarrolladores
...
http://localhost:8080/api/auth/external/callback?provider=GitHubSi usas otro puerto o HTTPS (p. ej. perfil `https` de Visual Studio), registra **esa** URL exacta en GitHub y la misma en `ExternalAuth:GitHub:RedirectUri`.
...
Build desde src/Enjoy.API/Dockerfile



API ASP.NET Core en el puerto 8080 del contenedor (mapeado al host).
...
Base de datos enjoydb, usuario/contraseña definidos en el propio docker-compose.yml.
...
Logs y trazas; UI y ingestión OTLP.
...
Conexión desde herramientas en el host (pgAdmin, psql, API corriendo fuera de Docker con cadena a localhost:5433).
...
Ingestión OTLP desde el host (p. ej. perfil http en launchSettings).
...
Interfaz web de Seq (http://localhost:8081).
...
API: abrir http://localhost:8080/swagger (en entornos no productivos Swagger está habilitado).



Seq: abrir http://localhost:8081 e iniciar sesión con la contraseña de administrador inicial definida en el servicio Seq del docker-compose.yml (cambiarla en despliegues reales).



Postgres: desde el host, con cliente SQL, localhost:5433 y las credenciales del servicio enjoy-db.
...
docker compose up -d --build, abrir Swagger, probar login con usuario sembrado (Database:DevelopmentUsers).
...
API en Docker + herramientas en host contra localhost:8080 y DB en localhost:5433.
...
Revisar logs en Seq; correlacionar con trazas OTLP si están configuradas.
...
Registrar app en GitHub, alinear RedirectUri con ExternalAuth:GitHub y probar flujo external/...-login → callback.
...
Endpoints que usan Gemini requieren Gemini:ApiKey y Gemini:ModelId acordes a la API Gemini, y cuotas de API.
...
GET al endpoint de inicio de login externo del proveedor expuesto por la API (ver Swagger).



El usuario autoriza en GitHub/Google.



Redirección a /api/auth/external/callback con code y state.



La API emite tokens JWT según la implementación actual.
...
Error de puerto en uso al hacer up



Otro proceso ya escucha en 8080, 5433, 8081 o 5342



Liberar el puerto o cambiar el mapeo en docker-compose.yml y alinear cadena de conexión / OAuth / Seq según §4 y §5.





API no arranca tras up
...
docker compose ps, logs de enjoy-db
...
Token ausente o expirado; redirect HTTPS pierde header



Renovar token; en Docker suele usarse solo HTTP en 8080
...
URL incorrecta en Serilog



enjoy-seq vs localhost y puertos 5341/5342
...
Consola del proveedor y ExternalAuth
...
ConnectionStrings:ConnectionEnjoy en appsettings o appsettings.Docker.json
...
Informe de cobertura (HTML): el reporte generado con ReportGenerator (capa Dominio) está publicado en GitHub Pages: https://vicellobre.github.io/ChistesAPI/.



Unitarias: no se alcanzó una cobertura global cercana al 90 % por falta de tiempo; se cubrió en buena medida la capa Dominio, no el resto de capas por igual.
